...
public class LeafLoadingView extends View {
    ...
    private static final int LEAF_MARGIN = 9;
    //用于控制绘制的进度条距离右的距离
    private static final int RIGHT_MARGIN = 25;
    private int mLeafMargin, mRightMargin;
    //中等幅度大小
    ...
    public LeafLoadingView(Context context, AttributeSet attrs) {
        ...
        mLeafMargin = Utils.dipToPx(context, LEAF_MARGIN);
        mRightMargin = Utils.dipToPx(context, RIGHT_MARGIN);
    ...
    private void drawProgressAndLeafs(Canvas canvas) {
        ...
        mCurrentProgressPosition = mProgress / TOTAL_PROGRESS * mProgressWidth;

        if (mCurrentProgressPosition < mArcRadius) {
            Log.i(TAG, "drawProgressAndLeafs-->mCurrentProgressPostion=" + mCurrentProgressPosition
            + "  mProgress=" + mProgress + "  mArcRadius=" + mArcRadius);
            ...
            Log.i(TAG, "mProgress=" + mProgress + "  --mCurrentProgressPostion="
                + mCurrentProgressPosition + "  --mArcProgressWidth=" + mArcRadius);
            //绘制White RECT
            //绘制Orange ARC
            //绘制Orange RECT
            //这个层级绘制能让叶子感觉融入棕色进度条中
    ...
    private void drawLeafs(Canvas canvas) {
        mLeafRotateTime = mLeafRotateTime > 0 ? mLeafRotateTime : LEAF_ROTATE_TIME;
        ...
        for (int i=0; i<mLeafInfos.size(); i++) {
            ...
            if (currentTime > leaf.startTime && leaf.startTime != 0) {
                //绘制叶子,根据叶子的类型和当前的时间获得叶子的(X,Y)
                getLeafLocation(leaf, currentTime);
                //根据时间计算旋转角度
                canvas.save();
                //通过Matrix控制叶子旋转
                Matrix matrix = new Matrix();
                float transX = mLeafMargin + leaf.x;
                float transY = mLeafMargin + leaf.y;
                Log.i(TAG, "drawLeafs: leaf.x=" + leaf.x + "  leaf.y=" + leaf.y);
                matrix.postTranslate(transX, transY);
                //通过时间关联旋转角度,可以直接通过修改LEAF_ROTATE_TIME调整叶子旋转快慢
                float rotateFraction = ((currentTime - leaf.startTime) % mLeafRotateTime)
                        / (float)mLeafRotateTime;
                int angle = (int)(rotateFraction * 360);
                //根据叶子旋转方向确定叶子旋转角度
                int rotate = leaf.rotateDirection == 0?
    ...
    private void getLeafLocation(Leaf leaf, long currentTime) {
        ...
        mLeafFloatTime = mLeafFloatTime > 0 ? mLeafFloatTime : LEAF_FLOAT_TIME;
        if (intervalTime < 0) {
            return;
        ...
        leaf.y = getLoacationY(leaf);
    }

    private int getLoacationY(Leaf leaf) {
        //y=A(wx+Q)+h
    ...
    private void initBitmap() {
        mLeafBitmap = ((BitmapDrawable)mResources.getDrawable(R.drawable.leaf))
                .getBitmap();
        mLeafWidth = mLeafBitmap.getWidth();
        mLeafHeight = mLeafBitmap.getHeight();

        mOuterBitmap = ((BitmapDrawable)mResources.getDrawable(R.drawable.leaf_kuang))
                .getBitmap();
    ...
    private void initPaint() {
        ...
        mBitmapPaint.setFilterBitmap(true);//抗锯齿
        ...
        mOrangePaint.setColor(Color.BLUE);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mTotalWidth = w;
        mTotalHeight = h;
        mProgressWidth = mTotalWidth - mLeafMargin - mRightMargin;
        mArcRadius = (mTotalHeight - 2 * mLeafMargin) / 2;

        mOuterSrcRect = new Rect(0, 0, mOuterWidth, mOuterHeight);
        mOuterDestRect = new Rect(0, 0, mTotalWidth, mTotalHeight);

        mWhiteRectF = new RectF(mLeafMargin + mCurrentProgressPosition, mLeafMargin,
                mTotalWidth - mRightMargin, mTotalHeight - mLeafMargin);
        mOrangeRectF = new RectF(mLeafMargin + mArcRadius, mLeafMargin,
                mLeafMargin + mCurrentProgressPosition, mTotalHeight - mLeafMargin);
        mArcRectF = new RectF(mLeafMargin, mLeafMargin,
                mLeafMargin + 2 * mArcRadius, mTotalHeight - mLeafMargin);//正方形

        mArcRightLocation = mLeafMargin + mArcRadius;
    }
    ...
    private class Leaf {
        ...
        int rotateDirection;//旋转方向0代表顺时针,1代表逆时针
        long startTime;//起始时间
    }
